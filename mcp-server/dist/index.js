...
class RuvFannMCPServer {
    ...
    async handlePredictOutcome(args) {
        try {
            const prediction = await this.toolInterceptor.interceptToolRequest(args.tool, args.params, args.context || {});
            return {
                content: [
                    {
                        type: "text",
    ...
    async handleLearnPattern(args) {
        try {
            await this.toolInterceptor.recordOutcome(args.tool, args.params, args.outcome, args.duration, args.error);
            return {
                content: [
                    {
                        type: "text",
    ...
    async handleGetSuggestions(args) {
        try {
            const suggestions = await this.patternPredictor.generateAlternatives(args.tool, args.params, args.goal);
            return {
                content: [
                    {
                        type: "text",
    ...
    async handleGCPTool(toolName, args) {
    ...
    async recordGCPOutcome(toolName, params, response, duration, prediction, errorMessage) {
        try {
            ...
            await this.toolInterceptor.recordOutcome(toolName, params, outcome, duration, response?.error?.message || errorMessage);
        }
        catch (error) {
            logger.warn('Failed to record GCP outcome', { error: error.message });
        }
    }
    formatPredictionResponse(prediction, toolName, action) {
    ...
    formatSuccessResponse(result, prediction) {
        let response = '';
    ...
    formatErrorResponse(error, prediction) {
