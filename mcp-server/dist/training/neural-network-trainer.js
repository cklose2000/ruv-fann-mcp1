...
export class NeuralNetworkTrainer {
    logger = logger.child({ component: 'NeuralNetworkTrainer' });
    ...
    async trainNetwork(config) {
        ...
        try {
            ...
            const { trainInputs, trainTargets, valInputs, valTargets } = this.splitTrainingData(inputs, targets, config.validationSplit);
            ...
            const result = await this.performBatchTraining(trainInputs, trainTargets, valInputs, valTargets, config);
     ...
     */
    async evaluateNetwork(testPatterns) {
        ...
        this.logger.info('Evaluation complete', {
            ...
            categoryAccuracies: Object.keys(categoryAccuracies).map(cat => `${cat}: ${(categoryAccuracies[cat] * 100).toFixed(1)}%`).join(', ')
        });
    ...
    prepareBatchTrainingData(patterns) {
        const inputs = [];
        const targets = [];
    ...
    splitTrainingData(inputs, targets, validationSplit) {
    ...
    async performBatchTraining(trainInputs, trainTargets, valInputs, valTargets, config) {
        const trainingLoss = [];
    ...
    async validateNetwork(valInputs, valTargets) {
    ...
    evaluatePrediction(prediction, target) {
    ...
    getPatternCategoryCounts(patterns) {
        const counts = {};
        patterns.forEach(pattern => {
            const category = pattern.failureType || 'success';
...
//# sourceMappingURL=neural-network-trainer.js.map