...
export class PatternLearner {
    ...
    async recordExecution(execution) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
            stmt.run(id, execution.tool, JSON.stringify(execution.params), JSON.stringify(this.extractContext(execution)), execution.outcome, execution.outcome === "success" ? 1 : 0, execution.duration, execution.error || null, execution.timestamp.toISOString());
    ...
    async findSimilarPatterns(tool, params, context, limit = 10) {
        try {
    ...
    async learnSequencePatterns(executions) {
    ...
    async learnFailurePatterns(executions) {
    ...
    async learnTimingPatterns(executions) {
    ...
    `);
        stmt.run(pattern.patternId, pattern.patternType, JSON.stringify(pattern.patternData), pattern.confidence, pattern.occurrenceCount, pattern.lastSeen.toISOString());
    }
    ...
    async getStatistics() {
        const stats = {
            ...
            patternTypes: this.db.prepare("SELECT pattern_type, COUNT(*) as count FROM learned_patterns GROUP BY pattern_type").all(),
        };
        return stats;
    }
    generateId(execution) {
