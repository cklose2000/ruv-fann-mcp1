...
export class RuvFannClient {
    ...
    fallbackFeatureExtraction(patterns, tool, params) {
        ...
        if (params.query) {
            ...
            const complexityScore = ((queryLower.includes('join') ? 0.2 : 0) +
        ...
        // Risk indicators
        const riskScore = ((params.datasetId && params.datasetId.includes('restricted') ? 0.5 : 0) +
        ...
        if (params.query) {
            const queryLower = params.query.toLowerCase();
            const structureScore = ((queryLower.includes('select') ? 0.3 : 0) +
