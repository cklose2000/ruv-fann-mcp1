...
export class GCPPredictor {
    ...
    patternStorage;
    logger;
    constructor(ruvFannClient, patternStorage) {
    ...
    async predictGCPOperation(tool, params, context = {}) {
        ...
        try {
            ...
            const [patternAnalysis, costAnalysis, authAnalysis, riskAnalysis] = await Promise.all([
    ...
    async analyzePatterns(tool, params, historicalCommands) {
    ...
    async analyzeCosts(tool, params) {
    ...
    async analyzeBigQueryCosts(params) {
        const warnings = [];
        let estimatedCost = 0;
        let confidence = 0.5;
    ...
    async analyzeAuthStatus(context) {
        const warnings = [];
        let authScore = 1.0;
    ...
    async analyzeRisks(tool, params, context) {
        const risks = [];
            ...
            if (params.dataset || params.datasetId) {
                ...
                if (userPatterns && userPatterns.frequent_datasets) {
                    const frequentDatasets = Array.isArray(userPatterns.frequent_datasets)
                        ...
                        : JSON.parse(userPatterns.frequent_datasets);
    ...
    async getNeuralPrediction(tool, params, context) {
        try {
    ...
    combinePredictions(analyses) {
        ...
        const successProbability = (patternAnalysis.successRate * weights.pattern +
    ...
    generateExplanation(analyses, successProbability) {
...
//# sourceMappingURL=gcp-predictor.js.map